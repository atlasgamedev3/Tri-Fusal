...
export class BombPuzzle extends Schema {
  @type("string") id: string = "";
  @type("string") label: string = "";
  @type("boolean") isSolved: boolean = false;
  @type("number") solvedAt: number = 0;
}
...
export class GameState extends Schema {
  ...
  @type("string") bombStatus: "ready" | "running" | "defused" | "detonated" = "ready";

  @type("number") bombDuration: number = 15 * 60;

  @type("number") bombRemaining: number = 15 * 60;

  @type("number") solvedPuzzleCount: number = 0;

  @type("number") requiredPuzzleCount: number = 0;

  @type([BombPuzzle]) bombPuzzles = new ArraySchema<BombPuzzle>();
