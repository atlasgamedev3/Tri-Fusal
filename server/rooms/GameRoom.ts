...
import { GameState, Player, Collectible, GridCell, PlayerColor, CollectibleType, CollectibleColor, CollectibleOrientation, Enemy, EnemyPersonality, BombPuzzle } from "../schema/GameState";
...
interface CompletePuzzleMessage {
  puzzleId: string;
}
...
export class GameRoom extends Room<GameState> {
  private readonly BOMB_DURATION = 15 * 60; // Tri-Fusal prototype round length.
  private readonly BOMB_PUZZLES = [
    { id: "puzzle-1", label: "Signal Locks" },
    { id: "puzzle-2", label: "Wire Sequence" },
  ];
  maxClients = 10;
  ...
  private readonly GAME_DURATION = this.BOMB_DURATION;
  ...
  onCreate(options: any) {
    ...
    this.onMessage("completePuzzle", async (client, message: CompletePuzzleMessage) => {
      const player = this.state.players.get(client.sessionId);
      if (!player || !this.state.gameStarted || this.state.countdown > 0 || this.state.isGameOver) return;
      if (this.state.bombStatus !== "running") return;

      const puzzle = this.state.bombPuzzles.find((item) => item.id === message.puzzleId);
      if (!puzzle) {
        client.send("puzzleRejected", { puzzleId: message.puzzleId, reason: "unknown" });
        return;
      }

      if (puzzle.isSolved) {
        client.send("puzzleSolved", { puzzleId: puzzle.id, solvedCount: this.state.solvedPuzzleCount });
        return;
      }

      puzzle.isSolved = true;
      puzzle.solvedAt = Date.now();
      this.state.solvedPuzzleCount = this.state.bombPuzzles.filter((item) => item.isSolved).length;

      this.broadcast("puzzleSolved", {
        puzzleId: puzzle.id,
        label: puzzle.label,
        solverColor: player.color,
        solvedCount: this.state.solvedPuzzleCount,
        requiredPuzzleCount: this.state.requiredPuzzleCount,
      });
      this.logEvent({ e: "puzzle", puzzleId: puzzle.id, p: this.userIds.get(client.sessionId) || client.sessionId });

      if (this.state.solvedPuzzleCount >= this.state.requiredPuzzleCount) {
        await this.defuseBomb();
      }
    });
  ...
  async onJoin(client: Client, options: any) {
    ...
    // Player name comes from the standalone main menu.
  ...
  private startGameplay() {
    ...
    this.countdownTimer = setInterval(() => {
      ...
      if (next <= 0) {
        ...
        this.state.bombRemaining = this.BOMB_DURATION;
  ...
  private startGameTimer() {
    this.gameTimer = setInterval(() => {
      ...
      this.state.bombRemaining = Math.max(0, this.state.timeRemaining);
      ...
      if (this.state.timeRemaining <= 0 && !this.state.isGameOver && !this.isDevMode) {
        void this.detonateBomb();
      }
    }, 1000);
  }

  private initializeBombRound() {
    this.state.bombPuzzles.clear();
    for (const puzzleDef of this.BOMB_PUZZLES) {
      const puzzle = new BombPuzzle();
      puzzle.id = puzzleDef.id;
      puzzle.label = puzzleDef.label;
      this.state.bombPuzzles.push(puzzle);
    }
    this.state.requiredPuzzleCount = this.state.bombPuzzles.length;
    this.state.solvedPuzzleCount = 0;
    this.state.bombDuration = this.BOMB_DURATION;
    this.state.bombRemaining = this.BOMB_DURATION;
    this.state.timeRemaining = this.BOMB_DURATION;
    this.state.bombStatus = "ready";
  }

  private async defuseBomb() {
    if (this.state.isGameOver || this.state.bombStatus === "defused") return;
    this.state.bombStatus = "defused";
    this.state.bombRemaining = Math.max(0, this.state.timeRemaining);
    this.broadcast("bombDefused", {
      remaining: this.state.bombRemaining,
      solvedCount: this.state.solvedPuzzleCount,
      requiredPuzzleCount: this.state.requiredPuzzleCount,
    });
    this.logEvent({ e: "defused", remaining: this.state.bombRemaining });
    await this.endGame();
  }

  private async detonateBomb() {
    if (this.state.isGameOver || this.state.bombStatus === "detonated") return;
    this.state.bombStatus = "detonated";
    this.state.bombRemaining = 0;
    this.broadcast("bombDetonated", {
      solvedCount: this.state.solvedPuzzleCount,
      requiredPuzzleCount: this.state.requiredPuzzleCount,
    });
    this.logEvent({ e: "detonated" });
    await this.endGame();
  }
  ...
  private updateHighScore(_score: number) {
    // High score lives in room state for the duration of the session.
  }
