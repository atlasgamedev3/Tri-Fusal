...
// Removed: ambient particles, NoiseBlobFieldCanvas — hidden behind opaque R3F canvas, wasted WebGL contexts
import { HudCornerLs, HUD_CHROME } from "@/components/ui/hud-chrome";
  ...
  return (
    <div className="relative h-3 min-w-0 flex-1">
      <div
        ...
        style={{
          borderColor: HUD_CHROME.barBorder,
          boxShadow: `inset 0 0 20px ${HUD_CHROME.barInset}`,
        }}
      >
        <div
...
export const GameScreen = ({
  ...
  const bombProgressPercent = bombDuration > 0
    ? Math.max(0, Math.min(100, (bombRemaining / bombDuration) * 100))
    : 0;
  const bombMinutes = Math.floor(bombRemaining / 60);
  const bombSeconds = Math.max(0, Math.floor(bombRemaining % 60)).toString().padStart(2, "0");
  const bombStatusLabel =
    bombStatus === "defused" ? "Defused" :
    bombStatus === "detonated" ? "Detonated" :
    bombStatus === "running" ? "Active" :
    countdown > 0 ? "Arming" : "Ready";
  ...
  return (
    <div className="tri-game-shell isolate w-full h-screen relative overflow-hidden bg-canvas">
      ...
      <div
        ...
        aria-hidden
      />
      {/* NOTE: ambient particles & NoiseBlobFieldCanvas removed —
                  ...
                  <div
                    ...
                    style={{
                      borderColor: barBloom > 0
                        ...
                        : HUD_CHROME.barBorder,
                      boxShadow: barBloom > 0
                        ? `inset 0 0 20px ${HUD_CHROME.barInset}, 0 0 ${16 + barBloom * 12}px rgba(255,255,255,${barBloom * 0.2})`
                        : `inset 0 0 20px ${HUD_CHROME.barInset}`,
                    ...
                    <div className="flex w-full -translate-y-1/2 items-center justify-end gap-0 pr-2 sm:pr-2.5">
                      ...
                      >
                        <div
                          ...
                          style={{
                            background: `linear-gradient(to right, ${HUD_CHROME.connectorFrom}, ${HUD_CHROME.connectorVia}, ${HUD_CHROME.connectorTo})`,
                          }}
                        />
                        <div
                          className="size-1.5 shrink-0 border border-solid"
                          style={{
                            backgroundColor: HUD_CHROME.marker,
                            borderColor: HUD_CHROME.markerRing,
                          }}
                        />
                      </div>
                    </div>
      ...
      <div className="tri-legacy-timer absolute bottom-4 left-1/2 z-10 -translate-x-1/2">
        <div
          ...
          style={{
            borderColor: bombStatus === "detonated"
        ...
        >
          <HudCornerLs />
          <div className="relative z-[1]">
            <p className="font-montreal text-[9px] uppercase leading-tight tracking-[0.12em] text-slate-300">Bomb Timer</p>
            <p className="mt-1 font-montreal text-3xl font-bold leading-none tracking-[-0.04em] text-white">
              {bombMinutes}:{bombSeconds}
            </p>
            <p className="mt-1 font-montreal text-[8px] uppercase tracking-[0.14em] text-slate-500">
              {bombStatusLabel}
            </p>
            <div className="mt-2 h-1 overflow-hidden rounded-none bg-white/10" aria-hidden>
              <div
                className="h-full transition-[width] duration-500"
                style={{
                  width: `${bombProgressPercent}%`,
                  background: bombStatus === "detonated"
                    ? "#f87171"
                    : bombStatus === "defused"
                      ? "#4ade80"
                      : "#e2e8f0",
      ...
      <div className="absolute right-4 top-[4.75rem] z-10 w-[min(18rem,calc(100vw-2rem))] sm:right-6">
        <aside
          ...
          style={{ borderColor: HUD_CHROME.border }}
          aria-label="Bomb module progress"
        >
          <HudCornerLs />
          <div className="relative z-[1]">
            <div className="mb-2 flex items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-0.5 font-montreal text-[12px] font-semibold leading-tight text-white">
                  {solvedPuzzleCount} / {requiredPuzzleCount || bombPuzzles.length} solved
                </p>
              </div>
              <p className="font-montreal text-[9px] uppercase tracking-[0.14em] text-slate-500">
                {bombStatusLabel}
              </p>
            </div>
            <div className="space-y-2">
              {bombPuzzles.length > 0 ? bombPuzzles.map((puzzle) => (
                <div
                  key={puzzle.id}
                  className="flex items-center justify-between gap-3 border border-white/10 bg-white/[0.03] px-2.5 py-2"
                >
                  <div className="min-w-0">
                    <p className="truncate font-montreal text-[12px] font-semibold leading-tight text-slate-100">
                      {puzzle.label || puzzle.id}
                    </p>
                    <p className="mt-0.5 font-montreal text-[9px] uppercase tracking-[0.13em] text-slate-500">
                  ...
                  {!isSpectator && !isGameOver && (
                    <button
                      type="button"
                      onClick={() => room?.send("completePuzzle", { puzzleId: puzzle.id })}
                      disabled={puzzle.isSolved || bombStatus !== "running" || countdown > 0}
                      className="shrink-0 border border-white/15 bg-white/5 px-2.5 py-1.5 font-montreal text-[9px] font-semibold uppercase tracking-[0.12em] text-white transition hover:bg-white/10 disabled:pointer-events-none disabled:opacity-40"
                    >
                      {puzzle.isSolved ? "Done" : "Solve"}
                    </button>
                  )}
                </div>
              )) : (
                <p className="font-montreal text-[11px] text-slate-500">Waiting for modules...</p>
              )}
            </div>
          </div>
        </aside>
            ...
            style={
              abandonVoteActive
                ? {
                    ...
                    borderColor: HUD_CHROME.border,
                    borderRadius: 0,
                    boxShadow: `inset 0 0 0 1px ${HUD_CHROME.border}`,
                    backdropFilter: "blur(4px)",
                    WebkitBackdropFilter: "blur(4px)",
                    animation: "hudVotePulse 1.2s ease-in-out infinite",
                  }
